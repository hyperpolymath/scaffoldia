...
# SCAFFOLDIA ABI/FFI Documentation
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libscaffoldia.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── scaffoldia.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── scaffoldia.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/scaffoldia.h
```
...
```c
#include "scaffoldia.h"

int main() {
    void* handle = scaffoldia_init();
    if (!handle) return 1;

    int result = scaffoldia_process(handle, 42);
    if (result != 0) {
        const char* err = scaffoldia_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    scaffoldia_free(handle);
    return 0;
}
```
...
Compile with:
```bash
gcc -o example example.c -lscaffoldia -L./zig-out/lib
```
...
```rust
#[link(name = "scaffoldia")]
extern "C" {
    fn scaffoldia_init() -> *mut std::ffi::c_void;
    fn scaffoldia_free(handle: *mut std::ffi::c_void);
    fn scaffoldia_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = scaffoldia_init();
        assert!(!handle.is_null());

        let result = scaffoldia_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libscaffoldia = "libscaffoldia"

function init()
    handle = ccall((:scaffoldia_init, libscaffoldia), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:scaffoldia_process, libscaffoldia), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:scaffoldia_free, libscaffoldia), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/scaffoldia.h
   ```
